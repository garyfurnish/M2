...
{
  pthread_cond_init(&exclusiveChangeCondition, NULL);
...
int M2File_GetThreadMode(M2File *file)
{
  return file->currentThreadMode;
}
...
{
  return new M2File(fileUnsyncState);
}
stdio0_fileOutputSyncState M2File_UnsyncState(M2File *file)
{
...
void M2File_ReleaseState(struct M2File *file)
{
  ...
  // in sync mode, release the recursive mutex
    ...
    if (it != file->threadStates.end()) {
      ...
      // this shouldn't happen, and if it does it is *clearly* an error and will lead to deadlock or memory corruption at some
      // point
      abort();
    }
    file->recurseCount -= 1;
