...
// There may be I/O bound threads, such as the the main interpreter thread.  So a good thing to set currentAllowedThreads to is
// the
// number of cores plus the expected number of I/O bound threads.
static int currentAllowedThreads = 2;

static void reverse_run(struct FUNCTION_CELL *p)
{
  if (p) {
    reverse_run(p->next);
    (*p->fun)();
...
struct ThreadTask *createThreadTask(
    const char *name, ThreadTaskFunctionPtr func, void *userData, int timeLimitExists, time_t timeLimitSeconds, int isM2Task)
{
...
ThreadTask::ThreadTask(
    ...
    : m_Name(name), m_Func(func), m_UserData(userData), m_Result(NULL), m_Done(false), m_Started(false), m_TimeLimit(timeLimit),
      m_Seconds(timeLimitSeconds), m_KeepRunning(true), m_CurrentThread(NULL), m_IsM2Task(isM2Task), m_ReadyToRun(false),
      m_Running(false)
{
...
ThreadSupervisor::ThreadSupervisor(int targetNumThreads) : m_TargetNumThreads(targetNumThreads), m_ThreadLocalIdCounter(1)
{
